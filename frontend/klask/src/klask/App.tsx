...
export default function App() {
  ...
  return <AppShell rightMode="team" rightLabel="Klask-4" showModeSwitch={false} saving={saving} error={error} onDismissError={() => setError(null)}>
    ...
    <hr /><h2>🎮 New game</h2>
    <select id="p1" value={p1Id} onChange={(e) => selectPlayer(1, e.target.value)}>{players.map((p: AnyRow) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><div><div>Score player 1</div><ScoreRow value={score1} onSelect={(v) => selectScore(1, v)} /></div>
    <select id="p2" value={p2Id} onChange={(e) => selectPlayer(2, e.target.value)}>{players.map((p: AnyRow) => <option key={p.id} value={p.id}>{p.name}</option>)}</select><div><div>Score player 2</div><ScoreRow value={score2} onSelect={(v) => selectScore(2, v)} /></div><button onClick={addMatch}>Save game</button>
